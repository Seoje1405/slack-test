...
export function FeedItem({ item, accentColor, showServiceBadge = false }: FeedItemProps) {
  ...
  const content = (
    <>
      ...
      <div className="flex-shrink-0 mt-0.5 relative">
        ...
        {showServiceBadge && (
          <span
            className="absolute -bottom-1 -right-1 w-3.5 h-3.5 rounded-full border border-[var(--bg-elevated)]"
      ...
      {/* 즐겨찾기 버튼 */}
      <div className="flex-shrink-0 flex items-center opacity-0 group-hover:opacity-100 transition-opacity mt-0.5">
        <button
  ...
  return (
    <div className="group px-4 py-2">
      {item.url ? (
        <Link href={item.url} target="_blank" rel="noopener noreferrer" className={innerClass}>
          {content}
        </Link>
      ) : (
